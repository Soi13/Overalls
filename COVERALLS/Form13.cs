...
namespace COVERALLS
{
    public partial class Form13 : Form
    {
        ...
        private void dataGridView1_DoubleClick(object sender, EventArgs e)
        {
            ...
            string id_norms_list = Convert.ToString(dataGridView1.CurrentRow.Cells[2].Value);
            ...
            string id_user = Convert.ToString(dataGridView1.CurrentRow.Cells[10].Value);
            string iais_naimen = Convert.ToString(dataGridView1.CurrentRow.Cells[12].Value);
            string id_iais = Convert.ToString(dataGridView1.CurrentRow.Cells[13].Value);

            if (f12.dataGridView2.Rows.Count > 0)
            {
                for (int s = 0; s <= f12.dataGridView2.Rows.Count - 1; s++)
                {
                    string iidd = Convert.ToString(f12.dataGridView2.Rows[s].Cells[0].Value);
                    string naim = Convert.ToString(f12.dataGridView2.Rows[s].Cells[2].Value);
            ...
            f12.dataGridView2.Rows.Add(id_position_coverall, id_sotrudnik, id_norms_list, naimen, edizm, kol_vo, per_isp, date_posled_vidachi, date_sled_vidachi, size, id_user, iais_naimen, id_iais);
            this.Close();
        }
    }
}
