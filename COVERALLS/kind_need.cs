...
namespace COVERALLS
{
    public partial class kind_need : Form
    {
        ...
        private void kind_need_Load(object sender, EventArgs e)
        {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                SystemSounds.Beep.Play();
                MessageBox.Show("Формирование заявки прошлым периодом запрещено!!!", "Внимание", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
                ...
                if ((Form2.val == 115) || (Form2.val == 3) || (Form2.val == 2) || (Form2.val == 1)) //ПОказ всех сотрудников если заходит Скворцов ОИ или Петушков ИВ или Шепилева
                {
                    ...
                    if (checkBox1.Checked == true)
                    {
                        ...
                        scmd4.CommandText = "declare @ID uniqueidentifier " +
                                            "set @ID=NEWID() " +
                                            "INSERT into NEED_MAIN (GUID, USER_ID, KIND_ZAYAV, PODR, PERIOD, DATETIME_CREATE) VALUES (@guid, @us_id, @kd_zayav, @podr, @per, GETDATE())";
                        ...
                        for (int i = 0; i <= ds.Tables[0].Rows.Count - 1; i++)
                        {
                            ...
                            for (int nr = 0; nr <= ds2.Tables[0].Rows.Count - 1; nr++)
                            {
                                ...
                                if (aa == 0)
                                {
                                    ...
                                    scmd6.CommandText = "INSERT into NEED_BODY (GUID, NAIMENOVAN_COVERALLS, CHARACTERISTIC_OVERALLS, DEPARTMENT, BRANCH, ED_IZM, KOL_VO, PRICE_EDINIC_PLAN, JAN, FEB, MAR, APR, MAY, JUN, JUL, AUG, SEP, OKT, NOV, DEC, DATETIME_CREATE, KONTRAGENT, FIO, SIZE_CLOTHES, SIZE_SHOES, HEIGHT) VALUES (@guid, @naim_cover2, @characterist, @dep, @br, @ed_iz, @kolvo, @pr_ed_pl, @jan, @feb, @mar, @apr, @may, @jun, @jul, @aug, @sep, @okt, @nov, @dec, GETDATE(), @kontr, @fio, @size_clo, @size_shoe, @height)";
                        ...
                        SystemSounds.Beep.Play();
                        MessageBox.Show("Потребность сформирована. Просмотреть ее возможно в списке потребностей.", "Информация", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                SystemSounds.Beep.Play();
                MessageBox.Show("Заявку на 1-й квартал формировать уже поздно. С данного момента имеется возможность подавать заявки только на 2-й квартал.", "Внимание", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            ...
            {
                SystemSounds.Beep.Play();
                MessageBox.Show("Заявку на 2-й квартал формировать уже поздно. С данного момента имеется возможность подавать заявки только на 3-й квартал.", "Внимание", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            ...
            {
                SystemSounds.Beep.Play();
                MessageBox.Show("Заявку на 3-й квартал формировать уже поздно. С данного момента имеется возможность подавать заявки только на 4-й квартал.", "Внимание", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            ...
            {
                SystemSounds.Beep.Play();
                MessageBox.Show("Заявку на 4-й квартал формировать уже поздно. С данного момента имеется возможность подавать заявки только на 1-й квартал следующего года.", "Внимание", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
